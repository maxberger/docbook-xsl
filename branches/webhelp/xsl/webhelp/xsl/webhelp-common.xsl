...
<xsl:param name="webhelp.common.dir">common/</xsl:param>
    ...
    <xsl:param name="callout.graphics.path"><xsl:value-of select="$webhelp.common.dir"
        />images/callouts/</xsl:param>
    <xsl:param name="callouts.extension">1</xsl:param>
    <xsl:param name="admon.graphics.path"><xsl:value-of select="$webhelp.common.dir"
        />images/admon/</xsl:param>
    <xsl:param name="admon.graphics" select="0"/>
    <!--xsl:param name="generate.toc">book toc</xsl:param-->

    <xsl:param name="generate.toc"> appendix toc,title article/appendix nop article toc,title book
        title,figure,table,example,equation chapter toc,title part toc,title preface toc,title
        qandadiv toc qandaset toc reference toc,title sect1 toc sect2 toc sect3 toc sect4 toc sect5
        toc section toc set toc,title </xsl:param>
    ...
    <xsl:template name="webhelpheader.logo">
        <a target="_blank">
            ...
            <img style="margin-right: 2px; height: 59px; padding-right: 25px; padding-top: 8px"
                align="right" src="{$webhelp.common.dir}images/logo.png"
                alt="{$brandname} Documentation"/>
        </a>
    </xsl:template>
                            ...
                            <xsl:if test="$webhelp.include.search.tab != 'false'">
                                <div id="searchDiv">
                                        ...
                                        <form onsubmit="Verifie(searchForm);return false"
                                            ...
                                            <fieldset class="searchFieldSet">
                                                ...
                                                <center>
                                                  ...
                                                  type="search" class="searchText" tabindex="1"/>
                                    ...
                                    <p class="searchHighlight">
                                        <a href="#" onclick="toggleHighlight()">Search Highlighter
                                            (On/Off)</a>
                                    </p>
                                </div>
                            </xsl:if>
    ...
    <xsl:template
        ...
        <xsl:if test="not(self::index) or (self::index and not($generate.index = 0))">
            ...
            <li>
                ...
                <xsl:if
                    test="part|reference|preface|chapter|bibliography|appendix|article|topic|glossary|section|simplesect|sect1|sect2|sect3|sect4|sect5|refentry|colophon|bibliodiv">
                    <ul>
                        <xsl:apply-templates
        ...
        <xsl:call-template name="write.chunk">
            ...
            <xsl:with-param name="content">
                <html>
                    ...
                    <body> If not automatically redirected, click <a href="content/{$default.topic}"
                                >content/<xsl:value-of select="$default.topic"/></a>
                    </body>
                </html>
            </xsl:with-param>
        </xsl:call-template>
